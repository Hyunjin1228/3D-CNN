...
def main(args): 
    ...
    for i in range(5):
        ...
        train_set = utdmadDataset(args.dataset_dir, type="train", transform = torchvision_transform, frames = args.frame, seed=seed, device=device)
        test_set = utdmadDataset(args.dataset_dir, type="test", transform = torchvision_transform, frames = args.frame, seed=seed, device=device)
...
if __name__ == "__main__":
    ...
    wandb.run.name = f'{args.dataset_dir}-{args.optim}-lr-{args.lr}-NE-{args.num_epochs}-aux'
    main(args)
    wandb.finish()